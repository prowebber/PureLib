var pureLib = (function () {
  ...
  function getTarget(selector, parent) {
    ...
    if (target && target.nodeName) return target; // Assume the user is trying to find a query selector after this point

    var dom = parent.querySelectorAll(target);
    if (dom) return dom; // If the target cannot be found
  ...
  function elem(selector) {
    ...
    var targets = getTarget(selector, parent); // Get the target element

    var isMultiElem = targets.length > 1; // True if the target element is multiple elements
    ...
    this.addClass = function (className) {
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          var _iterator2 = _createForOfIteratorHelper(className),
              _step2;
          ...
          try {
            for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
              ...
              _elem.classList.add(cssClass); // Add the class to the target element
